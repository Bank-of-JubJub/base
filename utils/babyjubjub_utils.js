...
export function privateToPublicKey(privateKey) {
    ...
    return {
        "x": _uint8ArrayToBigInt(babyJub.F.fromMontgomery(babyJub.F.e(publicKeyPoint[0])).reverse()),
...
export function exp_elgamal_encrypt(public_key, plaintext) {
...
export function add_points(P1, P2) {
    const Psum = babyJub.addPoint([babyJub.F.toMontgomery(_bigIntToUint8Array(P1.x).reverse()), babyJub.F.toMontgomery(_bigIntToUint8Array(P1.y).reverse())], [babyJub.F.toMontgomery(_bigIntToUint8Array(P2.x).reverse()), babyJub.F.toMontgomery(_bigIntToUint8Array(P2.y).reverse())]);
    return {
...
export function intToLittleEndianHex(n) {
...
export function packPublicKey(P1) {
    const montgomeryForm = [babyJub.F.toMontgomery(P1[0].reverse()), babyJub.F.toMontgomery(P1[1].reverse())];
