...
contract UsingEthSigners {
    ...
    mapping(bytes32 packedPublicKey => address erc4337Controller) public erc4337Controller;
    mapping(bytes32 packedPublicKey => MultisigParams signers) public multisigEthSigners;
    ...
    constructor(
        ...
        changeEthSignerVerifier = ChangeEthSignerVerifier(_changeEthSignerVerfier);
        changeMultisigEthSignerVerifier = ChangeMultisigEthSignerVerifier(_changeMultisigEthSignerVerifier);
    }

    // this can be use to add an eth signer or an erc4337 controller
    function addOtherController(bytes32 _packedPublicKey, address _ethAddress, bytes memory _proof) public {
        require(ethSigner[_packedPublicKey] == address(0x0), "eth signer already exists");
    ...
    function change4337Controller(bytes32 _packedPublicKey, address _newAddress) public {
        ...
        emit Change4337Controller(_packedPublicKey, msg.sender, erc4337Controller[_packedPublicKey]);
    }

    function changeEthSigner(bytes memory _proof, bytes32 _packedPublicKey, address _newEthSignerAddress) public {
        require(ethSigner[_packedPublicKey] == address(0x0), "eth signer must not exist");
        address signer = ethSigner[_packedPublicKey];

        bytes32 messageHash = keccak256(
            abi.encodePacked(address(this), _packedPublicKey, _newEthSignerAddress, otherNonce[_packedPublicKey])
        );
    ...
    function addMultisigEthSigners(
        ...
        require(ethSigner[_packedPublicKey] == address(0x0), "eth signer must not exist");
        require(multisigEthSigners[_packedPublicKey].ethSigners.length == 0, "multisig eth signer must not exist");

        multisigEthSigners[_packedPublicKey] = MultisigParams(_ethSignerAddresses, _threshold);
        ...
        emit AddMultisigEthSigners(_packedPublicKey, _ethSignerAddresses, _threshold);
    }
    ...
    function changeMultisigEthSigners(
        ...
        require(multisigEthSigners[_packedPublicKey].ethSigners.length > 0, "eth signer must exist");

        bytes32 messageHash = keccak256(
            abi.encodePacked(
                address(this), _packedPublicKey, _newEthSignerAddresses, _threshold, otherNonce[_packedPublicKey]
            )
        );
        // public inputs
        ...
        for (uint8 i; i <= 10; i++) {
            publicInputs[i] = bytes32(uint256(uint160(multisigEthSigners[_packedPublicKey].ethSigners[i])));
        }
        for (uint8 i = 0; i < 32; i++) {
            // Noir takes an array of 32 bytes32 as public inputs
        ...
        otherNonce[_packedPublicKey] += 1;
        multisigEthSigners[_packedPublicKey].ethSigners = _newEthSignerAddresses;
