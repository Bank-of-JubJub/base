...
export function exp_elgamal_encrypt(public_key: PointObject, plaintext: any) {
  ...
  if (
    ...
    const shared_secret = babyJub.mulPointEscalar(
      [
        babyJub.F.toMontgomery(_bigIntToUint8Array(public_key.x).reverse()),
        babyJub.F.toMontgomery(_bigIntToUint8Array(public_key.y).reverse()),
      ],
      randomness
    );
    ...
    const C1 = {
      x: _uint8ArrayToBigInt(
        babyJub.F.fromMontgomery(babyJub.F.e(C1P[0])).reverse()
      ),
      y: _uint8ArrayToBigInt(
        babyJub.F.fromMontgomery(babyJub.F.e(C1P[1])).reverse()
      ),
    };
    const C2 = {
      x: _uint8ArrayToBigInt(
        babyJub.F.fromMontgomery(babyJub.F.e(C2P[0])).reverse()
      ),
      y: _uint8ArrayToBigInt(
        babyJub.F.fromMontgomery(babyJub.F.e(C2P[1])).reverse()
      ),
    };
...
export function exp_elgamal_decrypt_embedded(
  ...
  const shared_secret = babyJub.mulPointEscalar(
    [
      babyJub.F.toMontgomery(_bigIntToUint8Array(C1.x).reverse()),
      babyJub.F.toMontgomery(_bigIntToUint8Array(C1.y).reverse()),
    ],
    private_key
  );
  const shared_secret_inverse = babyJub.mulPointEscalar(
    shared_secret,
  ...
  const plain_embedded = babyJub.addPoint(
    [
      babyJub.F.toMontgomery(_bigIntToUint8Array(C2.x).reverse()),
      babyJub.F.toMontgomery(_bigIntToUint8Array(C2.y).reverse()),
    ],
    shared_secret_inverse
  );
  return {
    x: _uint8ArrayToBigInt(
      babyJub.F.fromMontgomery(babyJub.F.e(plain_embedded[0])).reverse()
    ),
    y: _uint8ArrayToBigInt(
      babyJub.F.fromMontgomery(babyJub.F.e(plain_embedded[1])).reverse()
    ),
  };
}

export function add_points(P1: PointObject, P2: PointObject) {
  // Used for (homomorphic) addition of baby jubjub (encrypted) points
  const Psum = babyJub.addPoint(
    [
      babyJub.F.toMontgomery(_bigIntToUint8Array(P1.x).reverse()),
      babyJub.F.toMontgomery(_bigIntToUint8Array(P1.y).reverse()),
    ],
    [
      babyJub.F.toMontgomery(_bigIntToUint8Array(P2.x).reverse()),
      babyJub.F.toMontgomery(_bigIntToUint8Array(P2.y).reverse()),
    ]
  );
  return {
    x: _uint8ArrayToBigInt(
      babyJub.F.fromMontgomery(babyJub.F.e(Psum[0])).reverse()
    ),
    y: _uint8ArrayToBigInt(
      babyJub.F.fromMontgomery(babyJub.F.e(Psum[1])).reverse()
    ),
  };
}

export function intToLittleEndianHex(n: bigint) {
...
export function packPublicKey(P1: Point) {
  console.log(P1[0].reverse(), P1[1].reverse());

  const montgomeryForm: [Uint8Array, Uint8Array] = [
    babyJub.F.toMontgomery(P1[0].reverse()),
    babyJub.F.toMontgomery(P1[1].reverse()),
  ];
...
export function unpackPoint(point: Uint8Array): Point {
  let p = babyJub.unpackPoint(point);

  return [
    babyJub.F.fromMontgomery(babyJub.F.e(p[0])).reverse(),
    babyJub.F.fromMontgomery(babyJub.F.e(p[1])).reverse(),
  ];
}
