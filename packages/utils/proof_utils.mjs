...
export async function genProof(circuit_name, inputs) {
    ...
    switch (circuit_name) {
        case "lock":
            backend = new BarretenbergBackend(lock, { threads: 4 });
            noir = new Noir(lock, backend);
            break;
        case "transfer":
            backend = new BarretenbergBackend(transfer, { threads: 4 });
            noir = new Noir(transfer, backend);
            break;
        case "withdraw":
            backend = new BarretenbergBackend(withdraw, { threads: 4 });
            noir = new Noir(withdraw, backend);
            break;
        case "processPendingDeposits":
            backend = new BarretenbergBackend(process_pending_deposits, {
                threads: 4,
            });
            noir = new Noir(process_pending_deposits, backend);
            break;
        case "processPendingTransfers":
            backend = new BarretenbergBackend(process_pending_transfers, {
                threads: 4,
            });
            noir = new Noir(process_pending_transfers, backend);
            break;
        default:
...
const inputs_lock = {
    private_key: BigInt("2291123624948246627368989940774052753470489062495018070576418670157516550852"),
...
let pendingDepositProof = await genProof("processPendingDeposits", inputs_process_deposits);
