...
export class ProcessDepositCoordinator {
    ...
    async init() {
        ...
        await babyjub.init();
        this.startingAmount = encryptedBalanceArrayToEncryptedBalance((await privateToken.read.balances([this.to])));
        // if balance == 0, set the starting amount to encrypted 0
        if (this.startingAmount.C1x == 0n &&
            this.startingAmount.C1y == 0n &&
            this.startingAmount.C2x == 0n &&
            this.startingAmount.C2y == 0n) {
            this.startingAmount = encryptedValueToEncryptedBalance(getEncryptedValue(this.to, 0));
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (this.txsToProcess.length == 4)
                break;
        }
        // process the txs in the array, add them to the total amount
        for (let i = 0; i < this.txsToProcess.length; i++) {
            const pendingDeposit = (await privateToken.read.allPendingDepositsMapping([this.to, BigInt(this.txsToProcess[i])])); // [amount, fee]
        ...
        const C1 = babyjub.add_points({ x: this.startingAmount.C1x, y: this.startingAmount.C1y }, this.encryptedAmount.C1);
        const C2 = babyjub.add_points({ x: this.startingAmount.C2x, y: this.startingAmount.C2y }, this.encryptedAmount.C2);
        this.newBalance = {
            C1x: C1.x,
            C1y: C1.y,
            C2x: C2.x,
